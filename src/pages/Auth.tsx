      ...
      if (isSignUp) {
        ...
        if (signUpError) {
          if (signUpError.message.toLowerCase().includes('already registered') ||
            ...
            toast({ title: 'Info', description: 'هذا البريد مسجل مسبقاً، سيتم تسجيل الدخول تلقائياً...' });
    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      ...
      <div className="w-full max-w-4xl bg-card rounded-2xl shadow-2xl overflow-hidden grid md:grid-cols-2 md:min-h-[520px]">
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            {isSignUp && (
              <div>
                ...
                <Input type="text" value={fullName} onChange={(e) => setFullName(e.target.value)} required className="w-full" />
              </div>
            )}
            <div>
            ...
            <div>
              ...
              <Input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required className="w-full" minLength={6} />
            </div>
            <Button type="submit" className="w-full" disabled={loading} size="lg">
              {loading ? '...' : (isSignUp ? t('signUp') : t('signIn'))}
            </Button>
          </form>
          <button onClick={() => setIsSignUp(!isSignUp)} className="mt-6 text-sm text-muted-foreground hover:text-foreground transition-colors">
        ...
        <div className={`hidden md:flex flex-col items-center justify-center p-12 h-full bg-gradient-to-br from-primary via-primary to-accent transition-opacity duration-500 ease-in-out ${isSignUp ? 'md:order-1 opacity-100' : 'md:order-2 opacity-100'}`}>
          <img src={plLogo} alt="Premier League" className="w-64 h-auto mb-8 brightness-0 invert" />
        </div>
      </div>
    </div>
  );
